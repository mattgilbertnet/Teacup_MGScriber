...
void process_gcode_command() {
  uint32_t  backup_f;
	...
	if (next_target.seen_G) {
      ...
      case 46:
        //? --- M46: pressure on ---
        //? Using a "z max" endstop mounted in a way to sense when the scriber is pressing into the board
        home_z_positive();
        break;
      ...
      case 47:
        ...
        if(current_position.axis[Z] > Z_SCRIBER_LIFT * 1000){
          next_target.target.axis[Z] = Z_SCRIBER_LIFT * 1000;
          backup_f = next_target.target.F;
          next_target.target.F = MAXIMUM_FEEDRATE_X * 2L;
          enqueue(&next_target.target);
          next_target.target.F = backup_f;
